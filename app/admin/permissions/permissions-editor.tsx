        ...
        <table className="w-full text-sm border-collapse">
          <thead>
            <tr className="text-center text-gray-500 bg-gray-50">
              {/* Sticky Header Cell for Role Column */}
              <th className="sticky right-0 z-10 p-3 w-40 bg-gray-50 border-b border-gray-200 text-right font-semibold">
                نقش / فرم
              </th>
              {forms.map(f => (
                <th key={f.id} className="p-3 whitespace-nowrap border-b border-gray-200 border-r min-w-[300px]">
                  <div className="font-semibold text-gray-700">{f.titleFa}</div>
          ...
          <tbody>
            {roles.map((r, index) => (
              <tr key={r.id} className={`${index % 2 === 0 ? 'bg-white' : 'bg-gray-50/50'} hover:bg-blue-50/30 transition-colors`}>
                {/* Sticky Cell for Role Name */}
                <td className={`sticky right-0 z-10 p-3 font-medium whitespace-nowrap ${index % 2 === 0 ? 'bg-white' : 'bg-gray-50/50'} border-b border-gray-100 shadow-sm`}>
